...
请按照 AGENT_PROMPT.md 的工作流程执行任务。" \
    ...
    if [ "$NEW_COMMIT" != "$COMMIT" ]; then
        ...
        git push origin HEAD:feat/skill-manager-phase1 2>/dev/null || {
            echo "[Agent-${AGENT_ID}] 推送失败，尝试rebase后重推..."
            git fetch origin feat/skill-manager-phase1 2>/dev/null && git rebase origin/feat/skill-manager-phase1 2>/dev/null || true
            git push origin HEAD:feat/skill-manager-phase1 2>/dev/null || echo "[Agent-${AGENT_ID}] 推送仍失败，跳过"
        }
    else
