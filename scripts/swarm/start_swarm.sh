...
tmux new-session -d -s "$SESSION" -x 200 -y 50 \
    "AGENT_ID=1 WORK_DIR='$AGENT_DIR' LOG_DIR='$LOG_DIR' bash '$SWARM_DIR/agent_loop.sh'; read"

# 为后续agent创建新pane
for i in $(seq 2 "$NUM_AGENTS"); do
    AGENT_DIR="${PROJECT_DIR}-agent-${i}"
    tmux split-window -t "$SESSION" \
        "AGENT_ID=$i WORK_DIR='$AGENT_DIR' LOG_DIR='$LOG_DIR' bash '$SWARM_DIR/agent_loop.sh'; read"
    # 重新排列布局
...
echo ""
echo "查看状态:"
echo "  bash $SWARM_DIR/status.sh $PROJECT_DIR"
echo ""
echo "停止所有agent:"
echo "  bash $SWARM_DIR/stop_swarm.sh $PROJECT_DIR"
echo ""
